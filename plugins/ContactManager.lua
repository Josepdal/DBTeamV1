...
if matches[1] == "savecontact" and permissions(msg.from.id, msg.to.id, "ContactManager") then
    phone = matches[2]
...
if matches[1] == "remcontact" and permissions(msg.from.id, msg.to.id, "ContactManager") then
    del_contact("user#id"..matches[2], ok_cb, false)
    return "User #"..matches[2].." has been removed"
end

if matches[1] == "contactlist" and permissions(msg.from.id, msg.to.id, "ContactManager") then
 get_contact_list(get_contact_list_callback, {target = msg.from.id})
      return "ContactList Sended to your private"
end

if matches[1] == "dialoglist" and permissions(msg.from.id, msg.to.id, "ContactManager") then
    get_dialog_list(get_dialog_list_callback, {target = msg.from.id})
      return "DialogList Sended To Your Private"
      end

if matches[1] == "reload" and matches[2] == "all" and permissions(msg.from.id, msg.to.id, "ContactManager") then
